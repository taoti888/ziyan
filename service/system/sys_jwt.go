...
func (jwtService *JwtService) JsonInBlacklist(jwtList system.JwtBlacklist) (err error) {
	...
	err = global.GVA_REDIS.Set(context.Background(), jwtList.Jwt, "", 0).Err()
	return
}
...
func (jwtService *JwtService) IsBlacklist(jwt string) bool {
	_, err := global.GVA_REDIS.Get(context.Background(), jwt).Result()
	isNotFound := errors.Is(err, redis.Nil)
	return !isNotFound
...
func LoadAll() {
	...
	for i := 0; i < len(data); i++ {
		global.GVA_REDIS.Set(context.Background(), data[i], "", 0)
	}
}
